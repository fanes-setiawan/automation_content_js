...
export const generateVideoWithVeoStudio = async (prompt, env = {}) => {
  try {
    ...
    if (result && result.predictions && result.predictions.length > 0) {
      return result.predictions[0].videoUri || result.predictions[0].bytesBase64;
    } else {
      throw new Error('Tidak ada output video dari Veo AI Studio');
    }
  } catch (error) {
    if (error.response && error.response.data) {
      console.error('Kesalahan API Veo:', JSON.stringify(error.response.data, null, 2));
      throw new Error(`API Google menolak request: ${error.response.data.error?.message || error.message}`);
    }
